...
algebraic finish for $q\ge130$, and a finite integer cut envelope.  In
particular,
\[
...
larger; for example it gives $H(20)=51$, whereas the classical two-parameter
plaids give at most $50$.
...
orders are covered by an exact integer cut envelope, apart from five small
orders settled by direct exhaustive search.  Section~\ref{sec:odd} gives a
proof-complete account of the searches for Theorem~\ref{thm:oddmain}.
...
Classical plaids and their parity-swapped images form a subfamily, but the
four row/column parity counts above are independent.  The larger family is
occasionally better: it gives $H(20)=51$ and $H(24)=74$, while the best
classical plaids at those orders give $50$ and $72$.  At $n=16$, the profile
$(0,6,6,0)$ gives $36$ black-homogeneous and $32$ white-homogeneous cells
and is affinely equivalent to the classical $(4,4)$ plaid.
\end{remark}
...
agree with the terms of A279405 known before this work.}\label{tab:Hvals}
\end{table}
...
The margin in \eqref{eq:B35} is $0.01735\ldots$ per unit of $q$ against a fixed deficit of $9/4$, which is why the threshold sits at $q=130$ and not lower. The continuous picture fails at $n=18$ and $n=20$: the best purely continuous certificate gives $t\le43$ and $t\le53$ against the truth $42$ and $51$. The integrality step \eqref{eq:B29} --- and, in the finite range, the integer cut envelope of Section~\ref{sec:ladder} --- closes those two units.
\end{remark}
...
\end{equation}
where $E$ is the matrix of the moment equations.
...
\begin{proof}
The exact engine \texttt{envelope\_\bs{}checker.py} produces a complete
binary-tree record for every listed $q$.  Each record has no survivor or
unfinished box, binds both cut files by SHA-256, recomputes $H(2q)$, and
recounts the parity-separated witness cell by cell.  The companion checker
verifies all $122$ records and their coverage.  Lemma~\ref{lem:boxsound}
then gives the upper bound, and Proposition~\ref{prop:lower} gives equality.
\end{proof}
...
\end{center}
The following command, run from \texttt{anc/}, performs all portable
non-enumerative checks:
\begin{verbatim}
...
Python checks use only the standard library.  The C++ sources are C++20 and
were rebuilt with Apple Clang~21.0.0; the union-domain enumerator was also
reproduced with GCC~14 and Clang~17.  No floating-point comparison affects a
proof verdict.
...
\texttt{records\_\bs{}q021\_\bs{}q129/} contain the $122$ ladder records.
\texttt{check\_\bs{}envelope\_\bs{}records.py} checks their coverage,
terminal accounting, witnesses and bound-input hashes.  To re-establish an
order rather than audit its frozen record, run, for example,
\begin{verbatim}
python envelope_checker.py 21 22 23 --records-dir fresh-records
\end{verbatim}
The engine reads both cut files relative to the ancillary directory and uses
integer arithmetic after a denominator scale computed from the inputs.
...
The ladder records and exhaustive-search outputs are audit records: their
logical force is the corresponding short source together with the reductions
proved in Sections~\ref{sec:ladder} and~\ref{sec:odd}.  Re-running the source
re-establishes those finite computations.

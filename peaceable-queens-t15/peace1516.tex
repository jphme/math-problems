...
\end{tabular}
\medskip
\caption{The upper end of A279405. The values through $n=14$ are those recorded in \cite{OEIS279405}; $a(13)=16$ and $a(14)=25$ were contributed by Jaideep Padhi in June 2026. The values at $n=15$ and $n=16$ are Theorems~\ref{thm:main15} and~\ref{thm:main16}.}\label{tab:oeis}
\end{table}
...
Lemma~\ref{lem:budget} is an inequality and must be kept as one. It is tempting to argue that one may add lines to the black side until $S=2n$; this is false, because a line added to $R$ is removed from $R^{c}$ and can destroy $|\Wh|\ge m$. The witness of Proposition~\ref{prop:lower15} illustrates the point: its black side has $S=28$ and its white side $S=32$, and neither is $30$. Accordingly both searches below cover the whole admissible range of $S$, not merely the top stratum. For $n=16$ the point is even more visible: the necessary conditions of Section~\ref{sec:profiles16} admit no profile at all with $S\in\{30,31,32\}$.
\end{remark}
...
The proof is the exhaustive execution of Section~\ref{sec:completion15} on all $247$ profiles. The run statistics are given in Table~\ref{tab:stats15}; a per-profile certificate recording, for each profile, the fixed pair, the orbit count, a hash of the representative list, and the case counts, is included among the ancillary files.
...
\end{tabular}
\medskip
\caption{Statistics of the exhaustive run for $n=15$. The number of antidiagonal subsets examined equals $\sum_{\text{profiles}} (\text{orbit count})\cdot\binom{15}{a}$, which is checked row by row by the audit script.}\label{tab:stats15}
\end{table}
...
\subsection*{Stage 3: exact decision of \eqref{eq:subset16}}
...
The proof is the exhaustive execution of Section~\ref{sec:completion16} on all $677$ jobs. The run statistics are given in Table~\ref{tab:stats16}; a per-job certificate recording the oriented profile, the pair-orbit count, the number of admissible antidiagonal masks and the case counts is included among the ancillary files, for each of the two completion kernels.
...
antidiagonal subsets examined               & $13{,}163{,}028{,}768$ \\
survivors of the necessary-condition prefilter & $17{,}834$ \\
calls to the exact completion decision      & $17{,}834$ \\
branch nodes in those calls                 & $0$ \\
jobs admitting a $33+33$ configuration      & $0$ \\
\bottomrule
\end{tabular}
\medskip
\caption{Statistics of the exhaustive run for $n=16$. The number of antidiagonal subsets examined equals $\sum_{\text{jobs}}(\text{pair-orbit count})\cdot\binom{8}{a_0}\binom{8}{a_1}$, which is checked row by row by the audit script. The branch-node count is $0$ because the parity-aware extremal bound of Lemma~\ref{lem:exhaustive16} already refuted each of the $17{,}834$ surviving instances, so the depth-first search was never entered; the same $17{,}834$ instances were also refuted by direct enumeration of all admissible masks.}\label{tab:stats16}
\end{table}
...
\emph{Memory safety.} AddressSanitizer and UndefinedBehaviorSanitizer builds of both proof solvers pass all startup self-tests.
...
\emph{The witness.} The $20+20$ placement of Proposition~\ref{prop:lower15} was checked by four independently written scripts, including a from-scratch check that verifies all $400$ black--white pairs against all four attack relations directly, rather than through the line-set formulation.
...
\emph{The witness.} The $32+32$ placement of Proposition~\ref{prop:lower16} was verified independently of the line-set formulation by checking all $32\cdot 32=1024$ black--white pairs against all four attack relations directly, and separately by reconstructing the $(4,4)$-plaid from the definition in \cite[\S3]{CDHS24} and confirming that it coincides cell for cell with \eqref{eq:witness16} after the relabelling $i\mapsto i-1$.
...
To rebuild the upper bound for $n=15$:
\begin{verbatim}
...
To rebuild the upper bound for $n=16$:
\begin{verbatim}

...
\begin{proof}
The branch carries its own Laurent boundary quotient.  The target boundary
lies in the ideal of $\Z[z,z^{-1}]$ generated by the two one-tile boundary
polynomials, and cancelling their common quadratic factor in the Laurent
domain forces the scale $t$ of the equal side $X=ut$ into the residual
ideal, whose intersection with $\Z$ is $(v^2-u^2)$.  This is an ideal
membership, not merely a rationality statement: it gives $t=(v^2-u^2)r$
with $r$ an \emph{integer}, and inserting $X=u(v^2-u^2)r$ into the area
equation $X^2=Nu^2(v^2-u^2)$ of Theorem~\ref{thm:imp-g2a} gives the stated
square ray.  At $r=1$, the target boundary is too short to carry
...
\end{split}
\]
The common quadratic is the geometric direction relation.  Here $z$ is the
character of the direction lattice of this branch: under the standing
incommensurable-angle hypothesis $\alpha/\pi\notin\Q$ (rationality of
$\alpha/\pi$ together with $3\alpha+2\beta=\pi$ would make $\alpha/\beta$
rational), and every tile and target angle lies in $\Z\pi+\Z\gamma$, namely
$\alpha=2\gamma-\pi$, $\beta=2\pi-3\gamma$, and the shape-4 and shape-5
target angles $\alpha+\beta=\pi-\gamma$ and $\alpha+2\beta=3\pi-4\gamma$.
Every directed edge direction therefore has a unique expression
$\theta=j\pi+m\gamma$, $j\in\Z/2$, $m\in\Z$, and with
$\chi(\theta)=(-1)^jz^m$ the subdivision-and-cancellation argument in the
proof of Theorem~\ref{thm:laurent} applies verbatim: every tile contributes
$\pm z^kF$ or $\pm z^kG$, so, with $J=(q-pz,qz-p)$,
\[
 B(\partial T)\in(F,G)=Q\cdot J\subseteq\Z[z,z^{-1}],
\]
the ideal membership used for both shapes below.  The homomorphism
\[
...
Put $b=v^2-u^2$ and $Q(z)=uz^2-vz+u$.  First, $\alpha/\pi\notin\Q$:
otherwise $2\cos\alpha=v/u$ would be a rational algebraic integer and hence
an integer, whereas $1<v/u<2$ because $u<v<2u$.

Since $\beta=\pi-3\alpha$ and $\gamma=2\alpha$, every angle of the tile and
of the target lies in $\Z\pi+\Z\alpha$, so in a tiling of this branch every
directed edge direction has, once an anchor is chosen, a unique expression
\[
             \theta=j\pi+m\alpha,\qquad j\in\Z/2,\quad m\in\Z;
\]
uniqueness follows from $\alpha/\pi\notin\Q$ exactly as in
Section~\ref{sec:laurent}.  Define the character
$\chi(\theta)=(-1)^jz^m\in\Z[z,z^{-1}]^\times$ and the boundary value
$B(PQ)=|PQ|\,\chi(\theta)$ as there.  Anchor the $c$-edge of a direct tile
at $\theta=0$: traversing the edges $c,a,b$ counterclockwise, the exterior
angles $\pi-\beta=3\alpha$ and $\pi-\gamma$ place them in directions $0$,
$3\alpha$, $\pi+\alpha$, so the two one-tile boundary polynomials are, up to
Laurent units,
\[
\begin{split}
F(z)&=u^2z^3-bz+uv=(uz+v)\,Q(z)\qquad\text{(direct)},\\
G(z)&=uvz^3-bz^2+u^2=(u+vz)\,Q(z)\qquad\text{(reflected)};
\end{split}
\]
the reflected value arises from $F$ by $z\mapsto z^{-1}$ and orientation
reversal, and equals $G$ up to the Laurent unit $-z^{-3}$.  The
subdivision-and-cancellation argument in the proof of
Theorem~\ref{thm:laurent} applies verbatim: every tile contributes
$\pm z^kF$ or $\pm z^kG$, and interior segments cancel in pairs, so
\begin{equation}\label{eq:g2a-membership}
     B(\partial T)\in(F,G)=Q\cdot J,\qquad J=(uz+v,\ u+vz),
\end{equation}
an ideal membership in $\Z[z,z^{-1}]$.  For the residual ideal $J$,
\[
 v(uz+v)-u(u+vz)=b,\qquad
 \Z[z,z^{-1}]/J\cong\Z/b,\quad z\mapsto-vu^{-1},
\]
the evaluation being well defined because $\gcd(u,b)=1$; it kills both
generators, so $J\cap\Z=(b)$.

The target is isosceles with equal side $X$ and base $Y$, and the law of
sines gives $X=ut$, $Y=vt$ for a real scale $t>0$.  Anchored with its base
at $\theta=0$, its counterclockwise boundary value is
\[
 B(\partial T)=vt-utz-utz^{-1}=-tz^{-1}Q(z).
\]
By \eqref{eq:g2a-membership} there is an $h\in J$ with $-tz^{-1}Q=Qh$; the
Laurent ring over $\mathbb R$ is a domain, so $-tz^{-1}=h$.  In particular
the coefficient $t$ of the monomial $-tz^{-1}=h\in\Z[z,z^{-1}]$ is an
integer---as it also is by Lemma~\ref{lem:boundary-integrality} and
B\'ezout, since $X=ut$ and $Y=vt$ are integers and $\gcd(u,v)=1$---and
$t=-zh\in J$.  Hence
\[
                 t\in J\cap\Z=(b),\qquad t=br,\quad r\in\Z,\ r\ge1 .
\]
This is the force of the ideal membership: it yields $b\mid t$, not merely
$t\in\Q$.  Beeson's area equation $X^2=Nu^2b$ (Theorem~\ref{thm:imp-g2a})
then gives $N=u^2t^2/(u^2b)=br^2$ with $r$ an integer, the ray of
Proposition~\ref{prop:gamma2alpha}.
